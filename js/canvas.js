export class Canvas {
  constructor(el) {
    this.el = el;
    this.bubbles = [];
    this.lines = [];

    // 캔버스 이동(Pan)을 위한 좌표
    this.panX = 0;
    this.panY = 0;

    // 속도 계산을 위한 이전 프레임 좌표
    this.prevPanX = 0;
    this.prevPanY = 0;
    this.mouseX = 0;
    this.mouseY = 0;
    this.prevMouseX = 0;
    this.prevMouseY = 0;

    // 내부 컨텐츠 래퍼 (이동 대상)
    this.contentArea = document.createElement("div");
    this.contentArea.style.position = "absolute";
    this.contentArea.style.width = "100%";
    this.contentArea.style.height = "100%";
    this.contentArea.style.top = "0";
    this.contentArea.style.left = "0";
    // 3D 가속 힌트
    this.contentArea.style.transformOrigin = "0 0";
    this.el.appendChild(this.contentArea);

    this.svg = this.createSVG();
    this.contentArea.appendChild(this.svg);

    this.initGrab();
    this.loop();
  }

  createSVG() {
    const svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
    svg.style.position = "absolute";
    svg.style.width = "100%"; // 래퍼 기준
    svg.style.height = "100%";
    svg.style.pointerEvents = "none";
    // SVG 크기가 캔버스 이동에 따라 잘리지 않도록 아주 크게 하거나
    // translate 방식이므로 100%면 충분함 (래퍼가 이동하니까)
    // 단, 래퍼 밖으로 나가는 선 처리를 위해 overflow visible
    svg.style.overflow = "visible";
    return svg;
  }

  addBubble(b) {
    this.bubbles.push(b);
    this.contentArea.appendChild(b.el);
  }

  connect(parent, child) {
    const line = document.createElementNS("http://www.w3.org/2000/svg", "line");
    line.setAttribute("stroke", "#9ca3af");
    line.setAttribute("stroke-width", "2");
    this.svg.appendChild(line);
    parent.children.push(child);
    this.lines.push({ line, parent, child });
  }

  updateLines() {
    this.lines.forEach(({ line, parent, child }) => {
      line.setAttribute("x1", parent.x);
      line.setAttribute("y1", parent.y);
      line.setAttribute("x2", child.x);
      line.setAttribute("y2", child.y);

      // Update Highlight Style directly here or manage externally?
      // Managing externally matches separation of concerns better (app logic vs rendering).
      // However, if we want to animate or frequent updates, here is good.
      // For static selection, external is fine.
    });
  }

  initGrab() {
    let dragging = false, ox, oy;

    this.el.addEventListener("mousedown", e => {
      // 버블이나 다른 UI 클릭 시엔 이동 안함
      if (e.target !== this.el && e.target !== this.contentArea) return;

      dragging = true;
      ox = e.clientX;
      oy = e.clientY;
      this.el.style.cursor = "grabbing";
    });

    window.addEventListener("mousemove", e => {
      this.mouseX = e.clientX;
      this.mouseY = e.clientY;

      if (!dragging) return;
      const dx = e.clientX - ox;
      const dy = e.clientY - oy;

      this.panX += dx;
      this.panY += dy;

      this.updateTransform();

      ox = e.clientX;
      oy = e.clientY;
    });

    window.addEventListener("mouseup", () => {
      dragging = false;
      this.el.style.cursor = "";
    });
  }

  updateTransform() {
    this.contentArea.style.transform = `translate(${this.panX}px, ${this.panY}px)`;
  }

  removeBubble(bubble) {
    // 1. 버블 배열에서 논리적 제거 (즉시)
    this.bubbles = this.bubbles.filter(b => b !== bubble);

    // 2. 연결된 선 제거 (부모/자식 관계 끊기)
    this.lines = this.lines.filter(item => {
      const isConnected = item.parent === bubble || item.child === bubble;
      if (isConnected) {
        if (item.line.parentNode) {
          // 선은 즉시 제거하거나, 얘도 페이드아웃 줄 수 있음. 지금은 즉시 제거.
          item.line.parentNode.removeChild(item.line);
        }
        return false;
      }
      return true;
    });

    // 3. DOM 제거 (애니메이션 후)
    if (bubble.el) {
      bubble.el.classList.add("pop"); // 터지는 애니메이션 시작

      // CSS animation-duration(0.3s) 만큼 대기 후 삭제
      setTimeout(() => {
        if (bubble.el.parentNode) {
          bubble.el.parentNode.removeChild(bubble.el);
        }
      }, 300);
    }
  }

  loop() {
    // 1. Calculate Velocities (Screen Space)
    const mouseVx = this.mouseX - this.prevMouseX;
    const mouseVy = this.mouseY - this.prevMouseY;
    const panVx = this.panX - this.prevPanX;
    const panVy = this.panY - this.prevPanY;

    // 2. Prepare Interaction State
    // Mouse Pos in World Space
    const mouseWorldX = this.mouseX - this.panX;
    const mouseWorldY = this.mouseY - this.panY;

    const interaction = {
      mouse: {
        x: mouseWorldX,
        y: mouseWorldY,
        vx: mouseVx, // Velocity is direction, magnitude matters, scale doesn't change
        vy: mouseVy
      },
      pan: {
        vx: panVx,
        vy: panVy
      }
    };

    // 3. Update Bubbles
    this.bubbles.forEach(b => {
      if (b.tick) b.tick(interaction);
    });

    this.updateLines();

    // 4. Update Previous State
    this.prevMouseX = this.mouseX;
    this.prevMouseY = this.mouseY;
    this.prevPanX = this.panX;
    this.prevPanY = this.panY;

    requestAnimationFrame(() => this.loop());
  }
}
